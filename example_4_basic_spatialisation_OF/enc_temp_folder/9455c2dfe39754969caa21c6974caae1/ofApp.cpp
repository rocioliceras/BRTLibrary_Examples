...
void ofApp::setup() {
	...
	ofSetWindowShape(2*Width,Height+200);
	...
	axisY = ofGetHeight() - AxisImage.getHeight() - 70;
...
void ofApp::draw() {
	...
	ofPoint coordCenter = ofPoint(axisX + AxisImage.getWidth() / 2, axisY + AxisImage.getHeight() * 0.52);
	float scale = 0.35f;
...
void ofApp::mouseDragged(int x, int y, int button) {
	...
	case AZIMUTH_VOICE: {
		...
		if (x < azimuthCenterX - lineHalfLength)
			azimuthX = azimuthCenterX - lineHalfLength;
		...
		if (y < azimuthCenterY - lineHalfLength)
			azimuthY = azimuthCenterY - lineHalfLength;
		...
		// Calcular elevationX reflejando sobre el centro del eje azimuth
		elevationX = elevationCenterY + (azimuthCenterY - azimuthY);
	...
	case ELEVATION_VOICE: {
		...
		if (my < elevationCenterY - lineHalfLength)
			...
			elevationY = Height - my;

		calcular_elevacion(Width + elevationX, Height - elevationY);

		azimuthY = azimuthCenterY + (elevationCenterX - (elevationX + Width)); // inverso de elevationX
	...
	case AZIMUTH_STEPS: {
		...
		elevationX2 = elevationCenterY + (azimuthCenterY - azimuthY2);
	...
	case ELEVATION_STEPS: {
		...
		if (my < elevationCenterY - lineHalfLength)
			...
			elevationY2 = Height - my;

		calcular_elevacion(Width + elevationX2, Height - elevationY2);

		azimuthY2 = azimuthCenterY + (elevationCenterX - (elevationX2 + Width));
